...
Для того чтобы объявить модальное окно ассинхронным, необходимо вызвать метод `async`, передав в агрументе имя метода, который должен использоваться вместо `query`:
