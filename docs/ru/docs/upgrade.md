...
Ранее, чтобы избежать дублирования, элементы формы нужно было оборачивать в динамический компонент. 
Это позволяло управлять отображением — вертикальным или горизонтальным — и выглядело примерно так:
...
Начиная с версии `15.0`, такая обёртка больше не требуется. 
Теперь элементы формы следует использовать напрямую, без дополнительного компонента:

```blade
<input name=''>
```

Если в вашем шаблоне используется обёртка удалите ее для корректной работы.
