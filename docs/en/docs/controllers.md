...
You usually do not need to create custom controllers to do anything outside the package's scope since you can place [your views on the screen](/en/docs/custom-template/#views), no matter how complex they are.
...
To make the controller accessible through the Orchid interface, you need to declare a route for it in the route file (for example, `routes/platform`). This will ensure that common rules such as authorization apply to the controller.
